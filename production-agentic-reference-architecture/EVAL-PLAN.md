...
All four are currently caught. M4 is also the defence-in-depth demonstration: with the input
...
- **Structural groundedness only.** "Carries a citation" is not "is supported by the cited
  source"; closing that needs an entailment check (ADR-012, cut 4).
