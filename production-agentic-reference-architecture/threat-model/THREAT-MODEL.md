...
|---|---|---|---|
| **S**poofing | forged approval identity | **C20** approval requires an authenticated session held by a principal with the `approve:export` scope (`agent/identity.py`, ADR-015); the audit record names the authenticated principal id, not a display name | local credential store rather than SSO; production move is OIDC at the `verify_session` seam |
...
| **T**ampering | audit log rewritten | **C21** every record is hash-chained to its predecessor with a mirrored head pointer, so modification, reordering, deletion and truncation all fail verification (`agent/audit.py`, `run.py audit`) | a local attacker who rewrites the whole chain *and* the head pointer consistently is undetected without external storage — shipping records off-host remains the production requirement |
| **T**ampering | poisoned source silently changing conclusions | C2 neutralises instructions; **C23** every claim is verified against the text of the source it cites (coverage, numeric grounding, invented negation — ADR-016); coverage gaps declared | content that is factually wrong but not instruction-shaped passes — the architecture bounds *traceability*, not truth (ADR-001) |
| **T**ampering | misattributed citation — a fluent claim pointing at a source that does not support it | **C23**; a claim citing an unknown id is unsupported, not skipped | lexical support is not entailment (R10) |
| **S**poofing | stolen password used to approve an export | **C24** optional TOTP second factor; **C25** login throttling with exponential backoff, applied to correct passwords too | TOTP is opt-in, so an unenrolled principal is password-only |
| **T**ampering | forged approval from another origin (CSRF) | **C26** session-bound CSRF token compared in constant time, plus an independent Origin/Referer check | `http.server`; no per-form nonce rotation |
| **I**nformation disclosure | SSRF via a fetched URL reaching internal services | **C27** address checked *after DNS resolution*, refusing private/loopback/link-local/reserved ranges; redirects re-validated at every hop; non-http(s) schemes refused | a compromised allowlisted public host is still trusted |
| **D**enial of service | oversized or slow documents exhausting the fetcher | **C28** streaming size cap, connect and read timeouts, redirect cap, content-type checks | a slow-but-inside-timeout source still inflates p95 |
| **R**epudiation | "I never approved that export" | audit record: authenticated principal id, timestamp, report hash, destination, trace id, all inside a verified hash chain (C20 + C21) | password-based identity, so a shared credential still repudiates; MFA/SSO is the closing move |
...
| **D**enial of service | blocking a pending export by attempting a bad approval | **C22** a rejected gate attempt never mutates durable run state, so a failed approval leaves the run `awaiting_approval` (`_finish(persist_status=False)`) — found by running the demo, regression-tested | — |
...
6. Assume the destination is allowlisted. **Break 5 (C18 + C20)**: an *authenticated* human
   holding `approve:export` sees the report, its declared coverage gaps and the guardrail
   events that fired, before the write — and the export is recorded against their principal
   id in a hash-chained log (C21).
7. Every step above emits a span event, so even a fully-blocked attempt is visible on-call
   and in the eval.

Five independent breaks, of which four (C5, C6, C8, C20) are structural rather than
...
|---|---|---|---|---|
| ~~R1~~ | ~~Approval command is unauthenticated~~ — **CLOSED** (ADR-015): PBKDF2 credentials, scoped sessions, constant-time comparison, principal id in the audit record | — | — | residual: local credential store, no MFA; swap `verify_session` for OIDC |
| ~~R2~~ | ~~Audit log is not tamper-evident~~ — **CLOSED** (ADR-015): hash-chained records with a mirrored head pointer | — | — | residual: ship records to append-only external storage |
| ~~R3~~ | ~~Groundedness is structural, not entailment-based~~ — **CLOSED** (ADR-016): claims verified against the cited source's text via coverage, numeric grounding and invented-negation checks | — | — | residual: lexical support is not logical entailment; an NLI model remains the production recommendation |
| R4 | No rate limiting / per-user quota | Medium | Platform | per-principal quota upstream of the input screen |
| R5 | Regex-only PII detection — **highest open risk** | Medium | Architect | trained classifier at the output boundary |
| ~~R6~~ | ~~Pre-flight cost estimates are static~~ — **CLOSED** (ADR-016): `CostCalibrator` rolling mean over observed spend | — | — | residual: averages across question shapes |
...
| ~~R9~~ | ~~Review UI has no TLS, CSRF tokens or rate limiting~~ — **CLOSED** (ADR-016): CSRF tokens + Origin check, optional TLS, and the server refuses to bind off-loopback without it | — | — | residual: `http.server`, no account recovery, no failed-login audit |
| R4 | No rate limiting on *research requests* — **now the highest open risk alongside R5** | Medium | Platform | per-principal quota upstream of the input screen |
| R10 | Lexical support is not entailment; a claim copying a wrong source passes | Medium | Architect | NLI model or judge as a third output screen |
| R11 | `--retrieval http` has no search provider (ADR-012 cut 13) | Low | Architect | wire a commercial search API |
